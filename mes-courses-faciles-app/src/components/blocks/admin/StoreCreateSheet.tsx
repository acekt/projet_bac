...
import { createStoreAction } from '@/actions/ecommerce';
...
const storeSchema = z.object({
  ...
  description: z.string().max(300, "La description ne doit pas dépasser 300 caractères").optional(),
});

type StoreFormValues = z.infer<typeof storeSchema>;

interface StoreCreateSheetProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function StoreCreateSheet({ isOpen, onClose, onSuccess }: StoreCreateSheetProps) {
  ...
  const handleUploadFile = async (file: File) => {
    ...
    try {
      ...
      if (res.ok && data.url) {
        ...
        toast.error(data.error || "Une erreur est survenue lors de l'upload.");
      }
    } catch (err) {
  ...
  const onSubmit = async (data: StoreFormValues) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(`Le magasin ${data.name} a été créé.`);
        onSuccess();
        onClose();
      } else {
        setSubmitError(result.error || "Une erreur est survenue.");
        toast.error(result.error || "Impossible de créer le magasin.");
      }
    } catch (err: any) {
      console.error(err);
      ...
      >
        <SheetHeader className="p-6 pb-4 border-b border-slate-100 dark:border-slate-800">
          <SheetTitle className="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-2">
            <StoreIcon size={24} className="text-brand-primary" /> Nouveau Magasin
          </SheetTitle>
          <SheetDescription className="text-slate-500 dark:text-slate-400">
            Renseignez les détails pour ajouter un magasin partenaire sur la plateforme.
          </SheetDescription>
        </SheetHeader>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input 
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20 dark:focus:ring-brand-primary/10 transition-all outline-none text-foreground"
              />
              <AnimatePresence>
                {errors.name && (
                  <motion.p 
            ...
            <div className="space-y-2">
              ...
              <textarea 
                ...
                placeholder="Décrivez brièvement le magasin, ses horaires ou ses spécialités..."
                rows={4}

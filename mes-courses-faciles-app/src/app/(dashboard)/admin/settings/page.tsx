...
export default function AdminSettingsPage() {
  ...
  // Load profile and preferences data on mount
  useEffect(() => {
      ...
      try {
        ...
        if (profileRes.success && profileRes.admin) {
          setProfileForm({
            ...
            address: profileRes.admin.address || "",
          });
        }
        
        if (prefsRes.success && prefsRes.preferences) {
          setPreferencesForm({
            platformName: prefsRes.preferences.platformName || "MesCoursesFaciles",
            defaultDeliveryFee: prefsRes.preferences.defaultDeliveryFee ?? 1000,
            maintenanceMode: prefsRes.preferences.maintenanceMode ?? false,
            enableEmailNotifications: prefsRes.preferences.enableEmailNotifications ?? true,
            supportContact: prefsRes.preferences.supportContact || "",
          });
        }
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center py-32 space-y-4">
        ...
        <span className="text-sm font-bold text-slate-500 dark:text-slate-400">Chargement de la configuration...</span>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-in duration-300 relative overflow-y-auto h-full pr-2 pb-6">
      ...
      <Tabs defaultValue="profile" className="w-full space-y-6" onValueChange={setActiveTab}>
        ...
        <TabsList className="flex bg-slate-100/80 dark:bg-slate-900/60 p-1.5 rounded-2xl max-w-xl h-14 border border-slate-200/50 dark:border-slate-800/50 shadow-inner">
          <TabsTrigger 
            value="profile" 
            className="flex-1 rounded-xl text-xs sm:text-sm font-bold py-2.5 transition-all flex items-center gap-2 data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 data-[state=active]:text-slate-900 dark:data-[state=active]:text-white data-[state=active]:shadow-md border border-transparent"
          >
                    ...
                    <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-slate-100 dark:border-slate-850">
                      ...
                      <div className="flex items-start gap-3 p-4 rounded-2xl bg-slate-50/50 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-800/50">
                        ...
                        <div className="text-sm">
                          ...
                          <p className="text-slate-500 dark:text-slate-400 text-xs font-medium mt-1 leading-normal">
                            Envoyer des alertes e-mail automatiques aux administrateurs pour les nouvelles commandes.
                          </p>
                        </div>
                      </div>

                      <div className="flex items-start gap-3 p-4 rounded-2xl bg-rose-500/5 dark:bg-rose-500/5 border border-rose-500/10 dark:border-rose-950/20">
                        <div className="flex items-center h-5">
                          <input
